...
KissatResult KissatRunner::run(const std::string& input_path,
    ...
    if (WIFEXITED(code)) {
        ...
        int signal = WTERMSIG(code);
        throw std::runtime_error(
            "Kissat command terminated by signal " + std::to_string(signal) + ": " + command);
    } else {
        throw std::runtime_error(
            "Kissat command failed with unknown status: " + std::to_string(code) + ": " +
            command);
    }
#else
    int exit_code = code;
    ...
    if (exit_code != 0 && exit_code != 10 && exit_code != 20) {
        throw std::runtime_error(
            "Kissat command failed (exit code " + std::to_string(exit_code) + "): " + command);
    }
