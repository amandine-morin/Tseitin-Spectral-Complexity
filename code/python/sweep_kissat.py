...
def append_row(file_path: Path, row: list[object], write_header: bool) -> None:
    ...
    with file_path.open("a", newline="") as csvfile:
        ...
        if write_header:
            writer.writerow(["n", "d", "trial_index", "runtime_ms", "timeout_flag", "exit_code"])
        writer.writerow(row)
    ...
    for d in D_VALUES:
        for n in N_VALUES:
            for trial_index in range(1, TRIALS_PER_SETTING + 1):
                print(f"Running d={d}, n={n}, trial {trial_index}/{TRIALS_PER_SETTING}...")
                ...
                append_row(
                    RESULTS_PATH,
                    [n, d, trial_index, f"{runtime_ms:.3f}", timeout_flag, exit_code],
                    header_needed,
                )
                header_needed = False
