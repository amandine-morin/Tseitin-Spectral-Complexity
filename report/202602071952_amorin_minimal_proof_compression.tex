...
Empirical data (embedded PGFPlots) demonstrates a qualitative correlation between $\hdeg(G)$ and
...
and the Kissat CDCL solver, highlighting a sharp structure-dependent dichotomy at fixed $(n,d)$.
...
\subsection*{Updated experimental pipeline and controlled dichotomy (2026)}

We implemented a reproducible C++ pipeline for generating Tseitin instances, hashing the resulting
CNF at the byte level, and invoking the Kissat SAT solver with fixed resource limits. Instance
generation is deterministic given $(n,d,\texttt{seed},\texttt{graph\_mode})$, and solver exit semantics
are recorded explicitly (SAT / UNSAT / timeout).

At fixed size $n=80$ and degree $d=4$, we observe a sharp structure-dependent dichotomy across five seeds:
circulant graphs are consistently refuted quickly, whereas configuration-model graphs consistently reach
the timeout under the same solver and limits.

\begin{table}[h]
\centering\small
\caption{Controlled Tseitin instances at fixed size ($n=80$, $d=4$) using Kissat with a 60s timeout.}
\begin{tabular}{lcccc}
\toprule
Graph family & Seeds & Result & Runtime & Comment \\
\midrule
Circulant & 5 & UNSAT & $\sim$12 ms & consistently easy \\
Configuration model & 5 & timeout & 60 s & consistently hard \\
\bottomrule
\end{tabular}
\end{table}
